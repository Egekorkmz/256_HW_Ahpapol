...
try {
 ...
 function getNotifications($user_id){
    global $db ;
    $stmt = $db->prepare("select notification.sender_id, notification.timestamp, notification.type, users.first_name, users.last_name from `notification`, `users` where reciever_id = ? and notification.sender_id = users.user_id") ;
    $stmt->execute([$user_id]);
    return $stmt->fetchALL();
 }
